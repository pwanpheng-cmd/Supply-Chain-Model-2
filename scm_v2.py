...
def load_data():
    ...
    if os.path.exists(json_path):
        with open(json_path, encoding="utf-8") as f:
            return json.load(f)
    st.error("❌ ไม่พบไฟล์ scm_v2.json กรุณาวางไฟล์ไว้ในโฟลเดอร์เดียวกับ scm_streamlit.py")
    st.stop()
...
# ==================== PAGE: UPLOAD DATA ====================
    ...
    st.markdown('<p class="page-sub">อัปโหลดข้อมูลจริงเพื่ออัปเดตระบบ</p>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    for tab_obj, key, label in [
        ...
        with tab_obj:
            ...
            file = st.file_uploader(f"เลือกไฟล์ {label}", type=["csv","xlsx","xls"], key=f"up_{key}")
            if file:
                try:
                    df = pd.read_csv(file) if file.name.endswith(".csv") else pd.read_excel(file)
                    st.success(f"✅ โหลดสำเร็จ {len(df):,} records")
                    st.dataframe(df.head(10), use_container_width=True)
                    st.info("💡 เมื่อได้ข้อมูลจริง สามารถส่งให้ Claude re-process เพื่ออัปเดต scm_v2.json ได้เลยครับ")
